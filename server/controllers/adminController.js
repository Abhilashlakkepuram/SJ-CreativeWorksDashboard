...
const getAllAttendance = async (req, res) => {
    try {
        ...
        let startDate_abs, endDate_abs;
        if (date) {
            const d = new Date(date);
            startDate_abs = new Date(d.setHours(0, 0, 0, 0));
            endDate_abs = new Date(d.setHours(23, 59, 59, 999));
        } else {
            const range = getTodayRangeHelper();
            startDate_abs = range.start;
            endDate_abs = range.end;
        }

        // 🔴 ABSENT LOGIC (SPECIAL CASE)
        if (status === "absent") {
            const absentUsers = await User.aggregate([
                { $match: { ...userFilter, role: { $ne: "admin" }, isApproved: true } },
                {
                    $lookup: {
                        from: "attendances",
                        let: { userId: "$_id" },
                        pipeline: [
                            {
                                $match: {
                                    $expr: { $eq: ["$user", "$$userId"] },
                                    date: { $gte: startDate_abs, $lte: endDate_abs }
                                }
                            }
                        ],
                        as: "attendance"
            ...
            const absentRecords = absentUsers.map(u => ({
                _id: `absent-${u._id}-${startDate_abs.toISOString()}`,
                user: {
                    _id: u._id,
                    name: u.name,
                    email: u.email,
            ...
            // Pagination for absent (client side pagination simulation since it's an aggregation result)
            const total = absentRecords.length;
            const paginated = absentRecords.slice(skip, skip + parseInt(limit));
